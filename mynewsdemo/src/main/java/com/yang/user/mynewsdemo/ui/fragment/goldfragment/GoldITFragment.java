...
import com.yang.user.mynewsdemo.ui.adapter.goldadapter.GoldITAdapter;
...
public class GoldITFragment extends MyBaseFragment {
    ...
    GoldITAdapter adapter;
    List<GoldITBean.NewslistBean> mList = new ArrayList<>();
    ...
    protected void initData() {
        ...
        Call<GoldITBean> goldITBeanCall = goldApis.getGoldITBean("it", Constants.TIANXING_KEY, pagerNumberContent, currentpage);

        goldITBeanCall.enqueue(new Callback<GoldITBean>() {
            @Override
            public void onResponse(Call<GoldITBean> call, Response<GoldITBean> response) {
//                mList = response.body().getNewslist();
                mList.addAll(response.body().getNewslist());
                adapter = new GoldITAdapter(mContext, mList);
                recyclerView.setAdapter(adapter);
                adapter.notifyDataSetChanged();

                adapter.setOnItemClickListener(new GoldITAdapter.OnItemClickListener() {
                    @Override
            ...
            @Override
            public void onFailure(Call<GoldITBean> call, Throwable t) {
    ...
    protected void loadUpData() {
        myBaseSwipeRefreshLayout.setColorSchemeColors(Color.RED,Color.BLACK,Color.GREEN,Color.BLUE);
        ...
        myBaseSwipeRefreshLayout.setBottomColorSchemeColors(Color.GREEN,Color.BLACK,Color.BLUE,Color.YELLOW,Color.RED);
