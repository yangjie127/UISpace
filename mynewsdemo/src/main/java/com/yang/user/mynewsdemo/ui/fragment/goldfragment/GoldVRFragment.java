...
import com.yang.user.mynewsdemo.ui.adapter.goldadapter.GoldARAdapter;
...
public class GoldVRFragment extends MyBaseFragment {
    ...
    GoldARAdapter adapter;
    List<GoldVRBean.NewslistBean> mList = new ArrayList<>();
    ...
    protected void initData() {
        ...
        Call<GoldVRBean> goldITBeanCall = goldApis.getGoldVRBean("vr", Constants.TIANXING_KEY, pagerNumberContent, currentpage);

        goldITBeanCall.enqueue(new Callback<GoldVRBean>() {
            @Override
            public void onResponse(Call<GoldVRBean> call, Response<GoldVRBean> response) {

                Log.e("yangjie111", " " + mList.size());
//                mList = response.body().getNewslist();
                mList.addAll(response.body().getNewslist());
                adapter = new GoldARAdapter(mContext, mList);
                recyclerView.setAdapter(adapter);
                adapter.notifyDataSetChanged();

                adapter.setOnItemClickListener(new GoldARAdapter.OnItemClickListener() {
                    @Override
            ...
            @Override
            public void onFailure(Call<GoldVRBean> call, Throwable t) {
