...
import com.yang.user.mynewsdemo.ui.adapter.goldadapter.GoldNBAAdapter;
...
public class GoldNBAFragment extends MyBaseFragment {
    ...
    protected void initData() {
        ...
        Call<GoldNBABean> goldITBeanCall = goldApis.getGoldNBABean("nba", Constants.TIANXING_KEY, pagerNumberContent, currentpage);

        goldITBeanCall.enqueue(new Callback<GoldNBABean>() {
            @Override
            public void onResponse(Call<GoldNBABean> call, Response<GoldNBABean> response) {
//                mList = response.body().getNewslist();
                mList.addAll(response.body().getNewslist());
                adapter = new GoldNBAAdapter(mContext, mList);
                recyclerView.setAdapter(adapter);
                adapter.notifyDataSetChanged();

                adapter.setOnItemClickListener(new GoldNBAAdapter.OnItemClickListener() {
                    @Override
            ...
            @Override
            public void onFailure(Call<GoldNBABean> call, Throwable t) {
