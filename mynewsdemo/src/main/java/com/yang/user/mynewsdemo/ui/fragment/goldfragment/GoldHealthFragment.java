...
import com.yang.user.mynewsdemo.ui.adapter.goldadapter.GoldHealthAdapter;
...
public class GoldHealthFragment extends MyBaseFragment {
    ...
    GoldHealthAdapter adapter;
    List<GoldHealthBean.NewslistBean> mList = new ArrayList<>();
    ...
    protected void initData() {
        ...
        Call<GoldHealthBean> goldITBeanCall = goldApis.getGoldHealthBean("health", Constants.TIANXING_KEY, pagerNumberContent, currentpage);

        goldITBeanCall.enqueue(new Callback<GoldHealthBean>() {
            @Override
            public void onResponse(Call<GoldHealthBean> call, Response<GoldHealthBean> response) {
//                mList = response.body().getNewslist();
                mList.addAll(response.body().getNewslist());
                adapter = new GoldHealthAdapter(mContext, mList);
                recyclerView.setAdapter(adapter);
                adapter.notifyDataSetChanged();

                adapter.setOnItemClickListener(new GoldHealthAdapter.OnItemClickListener() {
                    @Override
            ...
            @Override
            public void onFailure(Call<GoldHealthBean> call, Throwable t) {
